...
module phi_matrix(
	...
	extend_valid_new #(12) extend_valid_new0(clk, rst, d_in_valid, valid_12);
	always @(posedge clk)
		if (rst) begin
			addra <= 0;
			cnt <= 0;
			...
			d_valid_12 <= valid_12;
			d2_valid_12 <= d_valid_12;
			cnt <= valid_12 ? (cnt==11 ? 0: cnt+1): 0;
			addra <= (valid_12 & cnt==0) ? start_cnt : (valid_12 ? addra+1:0);
			if (d2_valid_12) begin
				phi <= phi_tmp;
				out_valid <= d2_valid_12;
			end 
			else 
			begin
				phi <= 0;
