...
module payload_est_SINR #(parameter RX=8)(
	...
	always @(*) begin
		...
		for (i=0;i<RX;i=i+1) begin
			...
			sum_mean_data [34+2*$clog2(RX)-1:17+$clog2(RX)]= sum_mean_data [34+2*$clog2(RX)-1:17+$clog2(RX)] +{{$clog2(RX){mean_data_shift[33]}}, mean_data_shift[33:17]};
		end
	end
	...
	//Demodulation
	//=1 bpsk, =0 qpsk
	//sign real is high image is low
	assign payload= reg_uci_modulation ? (equal_sym[34+2*$clog2(RX)-1]&equal_sym[17+$clog2(RX)-1] ? 1: 0) : {equal_sym[17+$clog2(RX)-1], equal_sym[34+2*$clog2(RX)-1]} ;
